...
> [Maigret](https://github.com/soxoj/maigret) имеет огромный каталог — 3000+ сайтов.
> scratrace — иной подход: каталог меньше, но каждый сайт проверен вручную,
> с Playwright для SPA-сайтов, встроенным DuckDuckGo-доркингом и красивым
> интерфейсом. Мы вдохновлялись идеями Maigret и отдаём должное его авторам.

### 1. Ноль ложных срабатываний — гарантировано

Большинство OSINT-тулов (включая Maigret и форки Sherlock) считают сайт
«найденным», если он вернул **любой** HTTP 200, даже страницу
«пользователь не существует», отданную как положительный ответ. Мы — нет.
...
Если коды совпадают — сайт врёт → помечаем как ненадёжный (`type_url=None`).
Если коды различаются — сайт честный → запоминаем код детекта.
